      ...
      mysqlDB.query(sql, function(err, rows) {
        if (err) {
          ...
          if (rows.length === 0) {
            // 같은 값이 없으면 데베에 등록
            var sql2 =
              "INSERT INTO USER(platform_type,user_id,user_email) VALUES(?,?,?)";
...
router.post("/signin", function(req, res, next) {
  ...
  var sql =
    "select user_number, user_password from user where user_email='" +
    email +
    "'";
  mysqlDB.query(sql, function(err, rows) {
    if (err) {
      ...
      if (rows.length === 0) {
        // 같은 값이 없으면 로그인 실패 (아예 이메일도 등록이 안되있는 경우)
  ...
  mysqlDB.query(sql, function(err, rows) {
    if (err) {
      ...
      if (rows.length === 0) {
        // 같은 값이 없으면 데베에 등록
        var sql2 =
          "INSERT INTO USER(platform_type,user_email,user_password) VALUES(?,?,?)";
        ...
        // 이미 등록되어있는 이메일이면
        req.session.check = "alreadyEmail";
        res.redirect("/login");
      }
    }
  });
